(() => {
  const ID = 'dsh-model-mgr'
  window.__ModuleLoader__.load({
    id: ID,
    factory(require) {
      const React = require('react')
      const h = React.createElement
      const NS = 'llm-pi-ai'
      const SETTINGS_SLOT = 'settings.plugins.tab'
      const PROBE_PATH = '/plugins/dsh-model-mgr/probe'

      const control = {
        width: '100%', minHeight: 34,
        border: '1px solid color-mix(in srgb,currentColor 18%,transparent)',
        borderRadius: 7, background: 'transparent', color: 'inherit',
        padding: '0 8px', boxSizing: 'border-box',
      }
      const button = {
        minHeight: 32,
        border: '1px solid color-mix(in srgb,currentColor 18%,transparent)',
        borderRadius: 7, background: 'transparent', color: 'inherit',
        padding: '0 11px', cursor: 'pointer',
      }
      const iconButton = {
        width: 32, height: 32, padding: 0,
        border: '1px solid color-mix(in srgb,currentColor 16%,transparent)',
        borderRadius: 7, background: 'transparent', color: 'inherit', cursor: 'pointer',
        display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: '0 0 auto',
      }

      function at(root, path) {
        let value = root
        for (const key of path || []) {
          if (value == null || typeof value !== 'object') return undefined
          value = value[key]
        }
        return value
      }

      function findRow(list, modelId) {
        if (!Array.isArray(list)) return -1
        return list.findIndex(item => item?.id === modelId)
      }

      function modeOf(value) {
        if (!Array.isArray(value) || !value.includes('text')) return 'inherit'
        return value.includes('image') ? 'vision' : 'text'
      }

      function inputOf(mode) {
        return mode === 'vision' ? ['text', 'image'] : ['text']
      }

      function modeLabel(mode) {
        if (mode === 'vision') return '文本 + 图片'
        if (mode === 'text') return '纯文本'
        return '继承 DSH 默认/目录'
      }

      function useScope(scope) {
        const subscribe = React.useCallback(fn => scope.subscribe(fn), [scope])
        const read = React.useCallback(() => scope.getSnapshot(), [scope])
        return React.useSyncExternalStore(subscribe, read, read)
      }

      function Labelled({ label, hint, children }) {
        return h('label', { style: { display: 'grid', gap: 5, marginBottom: 12 } }, [
          h('span', { key: 'label', style: { fontSize: 13, fontWeight: 600 } }, label),
          h(React.Fragment, { key: 'control' }, children),
          hint ? h('span', { key: 'hint', style: { fontSize: 12, opacity: .68 } }, hint) : null,
        ])
      }

      function Chevron({ open }) {
        return h('svg', {
          width: 14, height: 14, viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': true,
          style: { transform: open ? 'rotate(90deg)' : undefined, transition: 'transform 120ms ease' },
        }, h('path', {
          d: 'M6 3.5L10.5 8L6 12.5', stroke: 'currentColor', strokeWidth: 1.5,
          strokeLinecap: 'round', strokeLinejoin: 'round',
        }))
      }

      function explicitModels(profile) {
        if (!Array.isArray(profile?.models)) return []
        return profile.models
          .filter(model => model && typeof model.id === 'string' && model.id.length > 0)
          .map(model => ({ id: model.id, name: model.name || model.id }))
      }

      function setOrDelete(object, key, value) {
        if (value === undefined) delete object[key]
        else object[key] = value
      }

      function modelLayers(effectiveProfile, userProfile, model) {
        const effectiveList = Array.isArray(effectiveProfile?.models) ? effectiveProfile.models : null
        const effectiveIndex = findRow(effectiveList, model.id)
        const effectiveStored = effectiveIndex >= 0
          ? effectiveList[effectiveIndex]
          : effectiveProfile?.modelOverrides?.[model.id]

        const userList = Array.isArray(userProfile?.models) ? userProfile.models : null
        const userIndex = findRow(userList, model.id)
        const userStored = userIndex >= 0
          ? userList[userIndex]
          : userProfile?.modelOverrides?.[model.id]

        return {
          explicit: effectiveIndex >= 0,
          effectiveStored,
          userList,
          userIndex,
          userStored,
          resolvedInput: effectiveStored?.input ?? model.input,
          resolvedContextWindow: effectiveStored?.contextWindow ?? model.contextWindow,
          resolvedMaxTokens: effectiveStored?.maxTokens ?? model.maxTokens,
        }
      }

      function buildModelOps(providerPath, list, index, modelId, mode, contextWindow, maxTokens) {
        if (!Array.isArray(providerPath) || providerPath.some(part => typeof part !== 'string')) {
          throw new Error('Provider settingsPath must contain only strings')
        }
        if (index >= 0) {
          if (!Array.isArray(list) || !list[index]) throw new Error('Explicit model row is no longer available in the user settings layer')
          const models = structuredClone(list)
          const row = { ...models[index] }
          setOrDelete(row, 'input', mode === 'inherit' ? undefined : inputOf(mode))
          setOrDelete(row, 'contextWindow', contextWindow)
          setOrDelete(row, 'maxTokens', maxTokens)
          models[index] = row
          return [{ op: 'set', path: [...providerPath, 'models'], value: models }]
        }

        if (typeof modelId !== 'string' || modelId.length === 0) throw new Error('Model id is required')
        const base = [...providerPath, 'modelOverrides', modelId]
        return [
          mode === 'inherit'
            ? { op: 'unset', path: [...base, 'input'] }
            : { op: 'set', path: [...base, 'input'], value: inputOf(mode) },
          contextWindow === undefined
            ? { op: 'unset', path: [...base, 'contextWindow'] }
            : { op: 'set', path: [...base, 'contextWindow'], value: contextWindow },
          maxTokens === undefined
            ? { op: 'unset', path: [...base, 'maxTokens'] }
            : { op: 'set', path: [...base, 'maxTokens'], value: maxTokens },
        ]
      }

      function buildCapacityResetOps(providerPath, list, index, modelId) {
        if (index >= 0) {
          if (!Array.isArray(list) || !list[index]) throw new Error('Explicit model row is no longer available in the user settings layer')
          const models = structuredClone(list)
          const row = { ...models[index] }
          delete row.contextWindow
          delete row.maxTokens
          models[index] = row
          return [{ op: 'set', path: [...providerPath, 'models'], value: models }]
        }
        const base = [...providerPath, 'modelOverrides', modelId]
        return [
          { op: 'unset', path: [...base, 'contextWindow'] },
          { op: 'unset', path: [...base, 'maxTokens'] },
        ]
      }

      async function probe(provider, model, kind) {
        const response = await fetch(PROBE_PATH, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ provider, model, kind }),
        })
        if (!response.ok) throw new Error(`HTTP ${response.status}`)
        return response.json()
      }

      function statusText(result, kind) {
        if (!result) return kind === 'vision' ? '⚠ 未测试' : '⚪ 未测试'
        if (result.ok) return `✅ ${kind === 'vision' ? '视觉通路正常' : '文本调用正常'}${result.latencyMs != null ? ` · ${result.latencyMs}ms` : ''}`
        if (result.layer === 'dsh') return `❌ DSH 层：${result.message || result.error?.message || '当前未声明图片输入'}`
        if (result.layer === 'provider') return `❌ Provider 层：${result.message || result.error?.message || '请求失败'}`
        if (result.layer === 'model') return `⚠ 通路成功 / 识别异常${result.text ? `：${result.text}` : ''}`
        return `❌ ${result.error?.message || result.message || '测试失败'}`
      }

      function ModelRow({ scope, provider, providerPath, model, effectiveProfile, userProfile, revision, writable }) {
        const layers = modelLayers(effectiveProfile, userProfile, model)
        const raw = layers.userStored
        const [open, setOpen] = React.useState(false)
        const [mode, setMode] = React.useState(() => modeOf(raw?.input))
        const [cw, setCw] = React.useState(() => raw?.contextWindow == null ? '' : String(raw.contextWindow))
        const [mt, setMt] = React.useState(() => raw?.maxTokens == null ? '' : String(raw.maxTokens))
        const [message, setMessage] = React.useState('')
        const [textResult, setTextResult] = React.useState(null)
        const [visionResult, setVisionResult] = React.useState(null)
        const [busy, setBusy] = React.useState(false)

        React.useEffect(() => {
          const next = modelLayers(effectiveProfile, userProfile, model).userStored
          setMode(modeOf(next?.input))
          setCw(next?.contextWindow == null ? '' : String(next.contextWindow))
          setMt(next?.maxTokens == null ? '' : String(next.maxTokens))
          setOpen(false)
        }, [revision, model.id])

        const disabled = busy || !writable
        const hasCapacityOverride = raw?.contextWindow != null || raw?.maxTokens != null
        const resolvedContext = layers.resolvedContextWindow
        const resolvedMax = layers.resolvedMaxTokens
        const resolvedMode = modeOf(layers.resolvedInput)

        function integer(value, label) {
          if (value.trim() === '') return undefined
          const parsed = Number(value)
          if (!Number.isSafeInteger(parsed) || parsed <= 0) throw new Error(`${label} 必须是正整数`)
          return parsed
        }

        function writeLocation() {
          if (!layers.explicit) return { list: null, index: -1 }
          if (!Array.isArray(layers.userList) || layers.userIndex < 0) {
            throw new Error('该显式 models 列表不属于用户设置层。为避免把 DSH 自动解析值固化，插件拒绝重建它；请先在原生“模型”页面保存一次该 Provider。')
          }
          return { list: layers.userList, index: layers.userIndex }
        }

        async function save() {
          if (!writable) return
          setBusy(true)
          setMessage('保存中…')
          try {
            const contextWindow = integer(cw, 'Context Window')
            const maxTokens = integer(mt, 'Max Tokens')
            const target = writeLocation()
            const ops = buildModelOps(providerPath, target.list, target.index, model.id, mode, contextWindow, maxTokens)
            const before = revision
            await scope.mutate(ops, before)
            const after = scope.getSnapshot().revision
            setMessage(after === before ? '⚠ 保存未生效，可能发生 revision 冲突；配置已刷新。' : '✅ 已保存；未填写的容量字段继续由 DSH 自动解析')
          } catch (error) {
            setMessage(`❌ ${error?.message || error}`)
          } finally {
            setBusy(false)
          }
        }

        async function clearCapacityOverrides() {
          if (!writable) return
          setBusy(true)
          setMessage('正在清除容量覆盖…')
          try {
            const target = writeLocation()
            await scope.mutate(buildCapacityResetOps(providerPath, target.list, target.index, model.id), revision)
            setMessage('✅ 已清除 contextWindow / maxTokens 显式覆盖，容量重新交给 DSH / catalog 解析')
          } catch (error) {
            setMessage(`❌ ${error?.message || error}`)
          } finally {
            setBusy(false)
          }
        }

        async function run(kind) {
          setBusy(true)
          try {
            const result = await probe(provider, model.id, kind)
            if (kind === 'text') setTextResult(result)
            else setVisionResult(result)
          } catch (error) {
            const result = { ok: false, layer: 'provider', error: { message: error?.message || String(error) } }
            if (kind === 'text') setTextResult(result)
            else setVisionResult(result)
          } finally {
            setBusy(false)
          }
        }

        const summary = [
          modeLabel(modeOf(raw?.input)),
          resolvedContext ? `DSH Context ${resolvedContext}` : null,
          resolvedMax ? `DSH Max ${resolvedMax}` : null,
          hasCapacityOverride ? '有手工容量覆盖' : null,
        ].filter(Boolean).join(' · ')

        const contextHint = `留空 = 不写入配置${resolvedContext ? `；DSH 当前解析值 ${resolvedContext}` : ''}`
        const maxHint = `留空 = 不写入配置${resolvedMax ? `；DSH 当前解析值 ${resolvedMax}` : ''}`
        const inputHint = `只修改这个模型。当前 DSH 解析：${modeLabel(resolvedMode)}。选择“继承”会清除模型自己的 input。`

        return h('div', { style: { borderTop: '1px solid color-mix(in srgb,currentColor 14%,transparent)' } }, [
          h('div', { key: 'row', style: { minHeight: 48, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0' } }, [
            h('div', { key: 'identity', style: { minWidth: 0, flex: 1 } }, [
              h('div', { key: 'name', style: { fontWeight: 650, wordBreak: 'break-all' } }, model.name || model.id),
              h('div', { key: 'summary', style: { marginTop: 3, fontSize: 12, opacity: .62, wordBreak: 'break-all' } }, `${model.name && model.name !== model.id ? `${model.id} · ` : ''}${summary}`),
            ]),
            h('button', { key: 'toggle', type: 'button', style: iconButton, 'aria-expanded': open, title: open ? '收起模型能力' : '展开模型能力', onClick: () => setOpen(value => !value) }, h(Chevron, { open })),
          ]),
          open ? h('div', { key: 'body', style: { padding: '6px 0 14px' } }, [
            hasCapacityOverride ? h('div', { key: 'capacity-warning', style: { padding: 10, marginBottom: 10, borderRadius: 8, background: 'color-mix(in srgb,#e7b35a 12%,transparent)', fontSize: 12 } }, [
              h('div', { key: 'text', style: { marginBottom: 8 } }, '这个模型在用户配置中存在显式 contextWindow / maxTokens。DSH 会优先使用这些值，而不是后续自动识别结果。'),
              h('button', { key: 'clear-capacity', style: button, disabled, onClick: clearCapacityOverrides }, '清除容量覆盖，恢复 DSH 自动识别'),
            ]) : null,
            h(Labelled, { key: 'mode', label: '输入能力', hint: inputHint }, h('select', { style: control, value: mode, disabled, onChange: event => setMode(event.currentTarget.value) }, [
              h('option', { key: 'inherit', value: 'inherit' }, '继承 DSH 默认/目录'),
              h('option', { key: 'text', value: 'text' }, '纯文本'),
              h('option', { key: 'vision', value: 'vision' }, '多模态（文本 + 图片）'),
            ])),
            h('div', { key: 'limits', style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 } }, [
              h(Labelled, { key: 'cw', label: 'Context Window', hint: contextHint }, h('input', { style: control, value: cw, disabled, placeholder: resolvedContext ? `DSH: ${resolvedContext}` : '留空 = DSH 自动识别', inputMode: 'numeric', onChange: event => setCw(event.currentTarget.value) })),
              h(Labelled, { key: 'mt', label: 'Max Tokens', hint: maxHint }, h('input', { style: control, value: mt, disabled, placeholder: resolvedMax ? `DSH: ${resolvedMax}` : '留空 = DSH 自动识别', inputMode: 'numeric', onChange: event => setMt(event.currentTarget.value) })),
            ]),
            h('div', { key: 'status', style: { display: 'grid', gap: 5, fontSize: 12, marginBottom: 10 } }, [
              h('span', { key: 'text' }, statusText(textResult, 'text')),
              h('span', { key: 'vision' }, statusText(visionResult, 'vision')),
            ]),
            h('div', { key: 'actions', style: { display: 'flex', flexWrap: 'wrap', gap: 8 } }, [
              h('button', { key: 'text', style: button, disabled: busy, onClick: () => run('text') }, '测试文本'),
              h('button', { key: 'vision', style: button, disabled: busy, onClick: () => run('vision') }, '测试视觉'),
              h('button', { key: 'save', style: button, disabled, onClick: save }, '保存'),
              h('button', { key: 'reset', style: button, disabled, onClick: () => { setMode('inherit'); setCw(''); setMt('') } }, '全部恢复继承'),
            ]),
            message ? h('div', { key: 'message', style: { fontSize: 12, marginTop: 8 } }, message) : null,
          ]) : null,
        ])
      }

      function countCapacityOverrides(userProfile) {
        let count = 0
        if (Array.isArray(userProfile?.models)) {
          for (const model of userProfile.models) if (model?.contextWindow != null || model?.maxTokens != null) count++
        }
        if (userProfile?.modelOverrides && typeof userProfile.modelOverrides === 'object') {
          for (const override of Object.values(userProfile.modelOverrides)) if (override?.contextWindow != null || override?.maxTokens != null) count++
        }
        return count
      }

      function ProviderPanel({ scope, catalog, entry }) {
        const snapshot = useScope(scope)
        const path = Array.isArray(entry?.settingsPath) ? [...entry.settingsPath] : []
        const effectiveProfile = snapshot.status === 'ready' && path.length ? at(snapshot.value, path) : undefined
        const userProfile = snapshot.status === 'ready' && path.length ? at(snapshot.user, path) : undefined
        const [open, setOpen] = React.useState(false)
        const [message, setMessage] = React.useState('')

        React.useEffect(() => { setOpen(false) }, [entry?.provider])

        if (!entry || entry.settingsNs !== NS) return null
        if (snapshot.status !== 'ready') return h('div', { style: { marginTop: 12, fontSize: 12, opacity: .72 } }, 'dsh-model-mgr：正在读取模型配置…')
        if (!path.length || !effectiveProfile || typeof effectiveProfile !== 'object') return null

        const group = (catalog?.groups || []).find(candidate => candidate.id === entry.provider)
        const catalogModels = Array.isArray(group?.models) ? group.models : []
        const localModels = explicitModels(effectiveProfile)
        const models = catalogModels.length > 0 ? catalogModels : localModels
        const writable = snapshot.writable !== false
        const legacyProviderVision = modeOf(userProfile?.defaultInput) === 'vision'
        const capacityOverrides = countCapacityOverrides(userProfile)

        async function clearLegacyDefault() {
          if (!writable) return
          setMessage('清理中…')
          try {
            await scope.mutate([{ op: 'unset', path: [...path, 'defaultInput'] }], snapshot.revision)
            setMessage('✅ 已清除系列级多模态默认')
          } catch (error) {
            setMessage(`❌ ${error?.message || error}`)
          }
        }

        return h('section', { style: { marginBottom: 10, padding: 14, border: '1px solid color-mix(in srgb,currentColor 16%,transparent)', borderRadius: 10 } }, [
          h('div', { key: 'head', style: { display: 'flex', alignItems: 'center', gap: 10 } }, [
            h('div', { key: 'title', style: { minWidth: 0, flex: 1 } }, [
              h('div', { key: 'name', style: { fontWeight: 800 } }, `${entry.displayName || entry.provider} · 模型能力`),
              h('div', { key: 'meta', style: { fontSize: 12, opacity: .62, marginTop: 3 } }, `${models.length} 个模型 · 按模型单独配置${capacityOverrides ? ` · ${capacityOverrides} 个容量覆盖` : ''}`),
            ]),
            h('button', { key: 'toggle', type: 'button', style: iconButton, 'aria-expanded': open, title: open ? '收起模型能力' : '展开模型能力', onClick: () => setOpen(value => !value) }, h(Chevron, { open })),
          ]),
          open ? h('div', { key: 'body', style: { marginTop: 10 } }, [
            capacityOverrides ? h('div', { key: 'capacity-note', style: { padding: 10, marginBottom: 10, borderRadius: 8, background: 'color-mix(in srgb,#e7b35a 10%,transparent)', fontSize: 12 } }, `检测到 ${capacityOverrides} 个模型存在显式容量覆盖。显式 contextWindow / maxTokens 会优先于 DSH 自动识别；请展开对应模型清理不需要的覆盖。`) : null,
            legacyProviderVision ? h('div', { key: 'legacy', style: { padding: 10, marginBottom: 10, borderRadius: 8, background: 'color-mix(in srgb,#e7b35a 12%,transparent)', fontSize: 12 } }, [
              h('div', { key: 'text', style: { marginBottom: 8 } }, '检测到这个 Provider 的 defaultInput 仍是“文本 + 图片”。这是旧版留下的系列级 fallback；当前版本只按模型配置。'),
              h('button', { key: 'clear', style: button, disabled: !writable, onClick: clearLegacyDefault }, '清除系列级多模态默认'),
            ]) : null,
            h('div', { key: 'intro', style: { fontSize: 12, opacity: .7, marginBottom: 8 } }, '插件现在区分“用户显式配置”和“DSH 解析结果”：只保存你实际填写的字段，不再把 DSH 自动解析的容量反写成固定值。'),
            message ? h('div', { key: 'message', style: { fontSize: 12, marginBottom: 8 } }, message) : null,
            models.length > 0
              ? models.map(model => h(ModelRow, {
                  key: model.id,
                  scope,
                  provider: entry.provider,
                  providerPath: path,
                  model,
                  effectiveProfile,
                  userProfile,
                  revision: snapshot.revision,
                  writable,
                }))
              : h('div', { key: 'empty', style: { fontSize: 12, opacity: .72, padding: '10px 0' } }, '当前 Provider 没有可枚举模型。'),
          ]) : null,
        ])
      }

      function ModelManager({ scope, loadCatalog }) {
        const snapshot = useScope(scope)
        const [catalog, setCatalog] = React.useState({ groups: [] })
        React.useEffect(() => {
          let active = true
          loadCatalog().then(value => { if (active) setCatalog(value) })
          return () => { active = false }
        }, [loadCatalog])

        if (snapshot.status !== 'ready') return h('div', { style: { padding: 18, opacity: .72 } }, '正在读取 llm-pi-ai 模型配置…')
        const providers = snapshot.value?.providers && typeof snapshot.value.providers === 'object' ? snapshot.value.providers : {}
        const entries = Object.entries(providers).map(([provider, profile]) => ({
          provider,
          displayName: profile?.displayName || provider,
          settingsNs: NS,
          settingsPath: ['providers', provider],
          active: true,
        }))

        return h('div', { style: { display: 'grid', gap: 10, padding: 2 } }, [
          h('div', { key: 'head', style: { marginBottom: 2 } }, [
            h('h3', { key: 'title', style: { margin: '0 0 6px' } }, '模型能力'),
            h('div', { key: 'hint', style: { fontSize: 12, opacity: .7 } }, '模型能力管理只在插件设置页提供。容量输入框留空时完全交给 DSH / catalog 自动解析。'),
          ]),
          entries.length > 0
            ? entries.map(entry => h(ProviderPanel, { key: entry.provider, scope, catalog, entry }))
            : h('div', { key: 'empty', style: { padding: 20, opacity: .7 } }, '当前没有已配置的 llm-pi-ai Provider。'),
        ])
      }

      const inject = ['slots', 'settingsScope', 'remote', 'remote.session']

      function apply(ctx) {
        const scope = ctx.settingsScope.bind({ namespace: NS })
        let cachedCatalog = { groups: [] }
        let catalogPromise = null

        const loadCatalog = () => {
          if (catalogPromise) return catalogPromise
          catalogPromise = Promise.resolve().then(async () => {
            const session = ctx.remote?.session
            if (!session || typeof session.modelCatalog !== 'function') return cachedCatalog
            const response = await session.modelCatalog()
            if (response?.ok && response.value) cachedCatalog = response.value
            return cachedCatalog
          }).catch(error => {
            console.warn('[dsh-model-mgr] model catalog unavailable; falling back to explicit settings models:', error)
            return cachedCatalog
          }).finally(() => { catalogPromise = null })
          return catalogPromise
        }

        ctx.slots.inject(SETTINGS_SLOT, () => ctx.slots.register({
          name: SETTINGS_SLOT,
          id: 'model-capabilities',
          order: 30,
          label: '模型能力',
          inject: () => ({ scope, loadCatalog }),
        }, ModelManager))
      }

      return { inject, apply, __test: { buildModelOps, buildCapacityResetOps, modelLayers } }
    },
  })
})()
