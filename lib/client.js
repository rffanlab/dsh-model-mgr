  ...
  window.__ModuleLoader__.load({
    ...
    factory(require) {
      ...
      function ModelRow({ scope, provider, providerPath, model, profile, revision, writable }) {
        ...
        React.useEffect(() => { setOpen(false) }, [model.id])
        ...
        return h('div', {
          style: { borderTop: '1px solid color-mix(in srgb,currentColor 14%,transparent)' },
        }, [
          h('div', {
            key: 'row',
            style: { minHeight: 48, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0' },
          }, [
      ...
      function ProviderPanel({ scope, catalog, entry, standalone = false }) {
        ...
        const bodyVisible = !standalone || standaloneOpen
        ...
        return h('section', {
          ...
          h('div', {
            ...
            h('div', { key: 'heading', style: { minWidth: 0, flex: 1 } }, [
              h('div', { key: 'title', style: { fontWeight: 800 } },
                standalone ? `${entry.displayName || entry.provider} · 模型能力` : '模型能力 · dsh-model-mgr'),
              standalone ? h('div', { key: 'meta', style: { fontSize: 12, opacity: .62, marginTop: 3 } }, `${models.length} 个模型`) : null,
            ]),
            standalone ? h('button', {
              key: 'toggle', type: 'button', style: iconButton, 'aria-expanded': standaloneOpen,
              title: standaloneOpen ? '收起 Provider' : '展开 Provider',
              onClick: () => setStandaloneOpen(value => !value),
            }, h(Chevron, { open: standaloneOpen })) : null,
          ]),
          bodyVisible ? h('div', { key: 'body', style: { marginTop: 10 } }, [
            h('div', { key: 'intro', style: { fontSize: 12, opacity: .7, marginBottom: 12 } },
              '模型行默认收起；点击右侧箭头再编辑能力或运行测试。'),
            h(Labelled, {
              key: 'default',
              label: '未声明模型的默认输入能力',
        ...
        return h('div', { style: { display: 'grid', gap: 10, padding: 2 } }, [
          h('div', { key: 'head', style: { marginBottom: 2 } }, [
            ...
            h('div', { key: 'hint', style: { fontSize: 12, opacity: .7 } },
              '兼容入口中的 Provider 默认也收起；展开后，模型行仍保持收起。'),
          ]),
          entries.length > 0
