  ...
  window.__ModuleLoader__.load({
    ...
    factory(require) {
      ...
      function ProviderPanel({ scope, catalog, entry, standalone = false }) {
        ...
        React.useEffect(() => { setPanelOpen(false) }, [entry?.provider])
        ...
        return h('section', {
          ...
          h('button', {
            ...
            style: {
              width: '100%', border: 0, padding: 0, margin: 0, background: 'transparent', color: 'inherit',
            ...
            h('div', { key: 'heading', style: { minWidth: 0, flex: 1 } }, [
              ...
              h('div', { key: 'meta', style: { fontSize: 12, opacity: .62, marginTop: 3 } },
                `${models.length} 个模型 · 默认收起，点击展开`),
            ]),
            h('span', { key: 'toggle', style: iconButton }, h(Chevron, { open: panelOpen })),
          ]),
          panelOpen ? h('div', { key: 'body', style: { marginTop: 10 } }, [
            h('div', { key: 'intro', style: { fontSize: 12, opacity: .7, marginBottom: 12 } },
              '模型能力面板默认收起；展开面板后，模型行仍默认收起。'),
            h(Labelled, {
              key: 'default',
              label: '未声明模型的默认输入能力',
        ...
        return h('div', { style: { display: 'grid', gap: 10, padding: 2 } }, [
          h('div', { key: 'head', style: { marginBottom: 2 } }, [
            ...
            h('div', { key: 'hint', style: { fontSize: 12, opacity: .7 } },
              'Provider 默认收起；展开 Provider 后，模型行仍保持收起。'),
          ]),
          entries.length > 0
